...
const loadProducts = async () => {
	try {
		...
		while (hasMore) {
			// 平台模式：查询所有商品（不传 only_my_products）
			const res = await getProductList({ page: page, size: pageSize })
			console.log(`[商品管理] 第${page}页API响应:`, res)
			...
			console.log(`[商品管理] 第${page}页解析出的商品数量:`, productList.length)
			
			// 如果返回的data为空数组，停止加载
			if (productList.length === 0) {
				console.log(`[商品管理] 第${page}页返回空数组，停止加载`)
				hasMore = false
				break
			}
			
			allProducts = [...allProducts, ...productList]
			console.log(`[商品管理] 累计商品数量:`, allProducts.length)
			
			// 继续加载下一页
			page++
		}
		
		console.log('[商品管理] 总共加载商品数量:', allProducts.length)
		...
		const filteredProductList = productList.filter(p => {
			...
			if (isBannerByName || isBannerBySku) {
				console.log(`[商品管理] 过滤掉轮播图商品: ${p.name} (ID: ${p.id})`)
				return false
			}
			return true
		})
		
		console.log('[商品管理] 过滤轮播图后商品数量:', filteredProductList.length, '原始数量:', productList.length)
		...
		console.log('[商品管理] 转换后的商品列表:', products.value)
	
	// 更新统计
	updateStats()
		
		if (products.value.length === 0) {
			console.warn('[商品管理] 商品列表为空')
		}
	} catch (error) {
		console.error('[商品管理] 加载商品列表失败', error)
		products.value = []
		updateStats()
...
const addProduct = () => {
	uni.navigateTo({ url: '/page1/merchant/product-add' })
}

const editProduct = (product) => {
	uni.navigateTo({ url: `/page1/merchant/product-add?id=${product.id}` })
}
