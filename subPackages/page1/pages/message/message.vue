...
const initMessageCategories = () => {
	...
	},
	{
		...
		title: '订单消息',
		icon: '📦',
		iconClass: 'icon-daifahuo',
		desc: '订单状态更新通知',
		unreadCount: 0,
		isPinned: savedStates['order']?.isPinned || false,
		translateX: 0,
		lastTimeStamp: savedStates['order']?.lastTimeStamp || (now - 30 * 1000) // 30秒前
	},
	{
		type: 'system',
		title: '系统通知',
	...
	},
	{
		...
		desc: '团队动态和成员变化',
		unreadCount: 0,
		isPinned: savedStates['team']?.isPinned || false,
		translateX: 0,
		lastTimeStamp: savedStates['team']?.lastTimeStamp || (now - 5 * 60 * 1000) // 5分钟前
	},
	{
		type: 'reward',
		title: '奖励消息',
		...
		desc: '推广奖励和补贴发放',
		unreadCount: 0,
		isPinned: savedStates['reward']?.isPinned || false,
		translateX: 0,
		lastTimeStamp: savedStates['reward']?.lastTimeStamp || (now - 10 * 60 * 1000) // 10分钟前
	}
	]
...
const updateCategoryUnreadCount = () => {
	...
	const allCategoryTypes = ['service', 'order', 'system', 'team', 'reward']
		...
		if (currentDeletedCategories.includes(type)) {
			...
			if (hasUnread) {
				...
				if (!existingCategory) {
					...
					const now = Date.now()
					const categoryMap = {
						service: { title: '联系客服', icon: '💬', iconClass: 'icon-xiaoxi', desc: '在线客服,随时为您服务', isService: true },
						order: { title: '订单消息', icon: '📦', iconClass: 'icon-daifahuo', desc: '订单状态更新通知', isService: false },
						system: { title: '系统通知', icon: '📢', iconClass: '', desc: '系统公告和重要提醒', isService: false },
						team: { title: '团队消息', icon: '👥', iconClass: 'icon-tuandui', desc: '团队动态和成员变化', isService: false },
						reward: { title: '奖励消息', icon: '💰', iconClass: 'icon-butie', desc: '推广奖励和补贴发放', isService: false }
					}
