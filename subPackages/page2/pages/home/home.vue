<template>
	<view class="home-page">
		...
		<view class="product-section">
			<view class="section-header">
				<view class="header-left">
					<text class="section-title">会员商品专区</text>
		...
		<view class="product-section">
			<view class="section-header">
				<text class="section-title">普通商品专区</text>
...
const loadBanners = async () => {
	try {
		...
		if (!bannerProduct) {
			console.log('[首页] 未找到轮播图商品')
			bannerList.value = []
			return
		}
		
		console.log('[首页] 找到轮播图商品，ID:', bannerProduct.id)
		...
		console.log('[首页] 从轮播图商品获取轮播图:', bannerImages.length, '张')
		...
		console.log('[首页] 轮播图加载成功:', bannerList.value.length, '张')
	} catch (error) {
...
const formatProduct = (product) => {
	...
	if (product.main_image) {
		...
		console.log(`[首页] 商品 ${product.id} 使用 main_image:`, image)
	}
	...
	if (!image) {
		...
		if (image) {
			console.log(`[首页] 商品 ${product.id} 使用 image_url/image:`, image)
		}
	}
	...
	if (!image) {
		...
		console.warn(`[首页] 商品 ${product.id} 没有图片，使用默认图片`)
	}
...
const goToProductList = (type) => {
	uni.navigateTo({ url: `/page1/product/list?type=${type}` })
}
