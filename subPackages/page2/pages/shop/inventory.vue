...
const loadProductList = async () => {
	try {
		...
		// 循环加载所有页面
		while (hasMore) {
			console.log(`[商家库存管理] 正在加载第 ${currentPage} 页商品...`)
			
			// 商家模式：调用 GET /api/users/{user_id}/products
			...
			if (products && products.length > 0) {
				...
				console.log(`[商家库存管理] 第 ${currentPage} 页加载了 ${products.length} 个商品，累计 ${allProducts.length} 个`)
			}
			...
			// 防止无限循环（最多加载100页）
			if (currentPage > 100) {
				console.warn('[商家库存管理] 已达到最大页数限制（100页），停止加载')
				hasMore = false
			}
		}
		
		console.log(`[商家库存管理] 共加载 ${allProducts.length} 个商品`)
		...
		products = products.filter(product => {
			...
			if (isBannerByName || isBannerBySku) {
				console.log(`[商家库存管理] 过滤掉轮播图商品: ${product.name} (ID: ${product.id})`)
				return false
			}
